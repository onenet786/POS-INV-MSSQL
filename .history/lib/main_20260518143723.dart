...
class AppStore extends ChangeNotifier {
  ...
  Invoice postInvoice({
    ...
    final invoice = Invoice(
      id: _nextInvoiceId,
      number: restaurantOrder?.orderNo ?? 'INV-$_nextInvoiceId',
      customer: customer,
      total: total,
      paid: paid,
      tendered: tendered,
      ...
      orderType: restaurantOrder?.type.label ?? '',
      orderReference: restaurantOrder?.reference ?? '',
    );
    _nextInvoiceId++;
...
Invoice _invoiceFromJson(Map<String, dynamic> json) {
  return Invoice(
    ...
    orderType: _string(json, 'orderType'),
...
class Invoice {
  ...
  bool get isRestaurantOrder => orderType.isNotEmpty || orderReference.isNotEmpty;
  String get orderLabel {
    if (orderType.isEmpty) return '';
    if (orderReference.isEmpty) return orderType;
    return '$orderType $orderReference';
  }
...
enum RestaurantOrderType {
  ...
  const RestaurantOrderType(this.label);
...
class _PosViewState extends State<PosView> {
    ...
    return CallbackShortcuts(
      bindings: {
        ...
        const SingleActivator(LogicalKeyboardKey.f8): () {
          if (cart.isNotEmpty) {
            unawaited(_postInvoice(store, total, tendered));
          }
        },
              ...
              content: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    onChanged: (_) => setDialogState(() {}),
                    onSubmitted: (_) {
                      final next = _restaurantOrderFromInput(type, reference.text, store);
  ...
  String _restaurantOrderNumber(
    ...
    final ref = cleaned.isEmpty ? (type == RestaurantOrderType.table ? '00' : 'TA') : cleaned;
    final suffix = store._nextInvoiceId.toString().padLeft(4, '0');
    return type == RestaurantOrderType.table ? 'T$ref-$suffix' : 'TA-$ref-$suffix';
  }
}
...
Future<void> _exportInvoice(
  ...
  _writeSimplePdf(
    ...
    rows: [
      ...
      if (invoice.isRestaurantOrder)
        ['Order', invoice.orderLabel, invoice.number],
      for (final line in lines)
        [
          line.product.name,
          line.quantity.toStringAsFixed(0),
...
String _invoiceReceiptText(
  ...
  pair('Date', date);
  if (invoice.isRestaurantOrder) pair('Order', invoice.orderLabel);
